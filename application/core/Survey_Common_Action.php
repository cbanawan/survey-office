...
class Survey_Common_Action extends CAction {
    ...
    protected function _renderWrappedTemplate($sAction = '', $aViewUrls = array(), $aData = array()) {
        ...
        if (!empty($aData['display']['menu_bars']['projects'])) {
            $this->_projectGroupBar(!empty($aData['ugid']) ? $aData['ugid'] : 0);
        }
        //nilesh
        if (!empty($aData['display']['menu_bars']['panellist'])) {
            $this->_panellistGroupBar(!empty($aData['ugid']) ? $aData['ugid'] : 0);
        }
        //end
        if (!empty($aData['display']['menu_bars']['country'])) {
            $this->_regionGroupBar(!empty($aData['ugid']) ? $aData['ugid'] : 0);
        }
        if (!empty($aData['display']['menu_bars']['contact_bars'])) {
            $this->_contactGroupBar(!empty($aData['ugid']) ? $aData['ugid'] : 0);
        }
        if (!empty($aData['display']['menu_bars']['cms_bars'])) {
            $this->_cmsGroupBar(!empty($aData['ugid']) ? $aData['ugid'] : 0);
        }
        ...
        if (!isset($aData['display']['footer']) || $aData['display']['footer'] !== false)
            Yii::app()->getController()->_getAdminFooter('http://docs.gowebsurvey.com', $clang->gT('GoWebSurvey online manual'));
    ...
    /**
     * Load menu bar of Projects controller.
     * @param int $ugid
     * @return void
     */
    function _projectGroupBar($ugid = 0) {
        ...
        $this->getController()->renderPartial('/admin/projects/projectsbar_view', $data);
    }

    /**
     * Load menu bar of Regioncontroller.
     * @param int $ugid
     * @return void
     */
    function _panellistGroupBar($ugid = 0) {
        $data['clang'] = Yii::app()->lang;
        Yii::app()->loadHelper('database');
        $data['ugid'] = $ugid;
        $data['imageurl'] = Yii::app()->getConfig("adminimageurl"); // Don't came from rendertemplate ?
        $this->getController()->renderPartial('/admin/panellist/panellistbar_view', $data);
    }

    function _regionGroupBar($ugid = 0) {
        $data['clang'] = Yii::app()->lang;
        Yii::app()->loadHelper('database');
        $data['ugid'] = $ugid;
        $data['imageurl'] = Yii::app()->getConfig("adminimageurl"); // Don't came from rendertemplate ?
        $this->getController()->renderPartial('/admin/region/country/countrybar_view', $data);
    }

    /**
     * Load menu bar of contact controller.
     * @param int $ugid
     * @return void
     */
    function _contactGroupBar($ugid = 0) {
        $data['clang'] = Yii::app()->lang;
        Yii::app()->loadHelper('database');
        $data['addcontact'] = 'contact';
        $data['imageurl'] = Yii::app()->getConfig("adminimageurl"); // Don't came from rendertemplate ?
        $this->getController()->renderPartial('/admin/contacts/contact_group/contactbar_view', $data);
    }
    ...
    function _getGroupBar($ugid = 0) {
        ...
        Yii::app()->loadHelper('database');
        $data['imageurl'] = Yii::app()->getConfig("adminimageurl"); // Don't came from rendertemplate ?
        $this->getController()->renderPartial('/admin/get/getbar_view', $data);
    }
